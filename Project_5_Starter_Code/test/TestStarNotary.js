...
it('can add the star name and star symbol properly', async() => {
    ...
    // Find StarId
    let starCreated = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal(contractName, 'StarToken');
    assert.equal(contractSymbol, 'STR');
    assert.equal(starCreated, 'awesome star');
});
...
it('lets 2 users exchange stars', async() => {
    ...
    let starId1 = 7;
    let starId2 = 8;
    await instance.createStar('awesome star 01', starId1, {from: user1});
    await instance.createStar('awesome star 02', starId2, {from: user2});
